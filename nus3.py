...
def mcode(ite):
    R = 0.3
    testfile = open("Nus%f_%f.txt" % (R,ite), 'w')
    ...
    # Y = Y.transpose()
    Y = np.ravel(Y)
    train_indices, test_indices = splitdata(X=X, Y=Y, ratio=0.13, seed=seed)
    X = X[train_indices]
    Y = Y[train_indices]
    Xnew1 = X[:, 0:64]
    Xnew2 = X[:, 65:290]
    ...
    testfile.write("Nus 0.1 RF:&%s pm%s &" % (floored_percentage(np.mean(esv),2), floored_percentage(np.std(esv),2)) + '\n')
