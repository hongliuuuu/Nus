...
for r in range(3):
    ...
    for l in range(10):
        ...
        # Y = Y.transpose()
        Y = np.ravel(Y)
        train_indices, test_indices = splitdata(X=X, Y=Y, ratio=0.13, seed=1000 + l)
        X = X[train_indices]
        Y = Y[train_indices]
        Xnew1 = X[:, 0:64]
        Xnew2 = X[:, 65:290]
    ...
    testfile.write("Nus 0.1:&%s\%\pm%s &" % (floored_percentage(np.mean(esv),2), floored_percentage(np.std(esv),2)) + '\n')
    testfile.write(" &%s\%\pm%s & " % (floored_percentage(np.mean(erelf),2), floored_percentage(np.std(erelf),2)) + '\n')
    testfile.write(" &%s\%\pm%s & " % (floored_percentage(np.mean(elfs),2), floored_percentage(np.std(elfs),2)) + '\n')
    testfile.write("&%s\%\pm%s & " % (floored_percentage(np.mean(erfsvm),2), floored_percentage(np.std(erfsvm),2)) + '\n')
    testfile.write(" &%s\%\pm%s & " % (floored_percentage(np.mean(e8),2), floored_percentage(np.std(e8),2)) + '\n')
    testfile.write(" &%s\%\pm%s &" % (floored_percentage(np.mean(elaterf),2), floored_percentage(np.std(elaterf),2)) + '\n')
    testfile.write(" &%s\%\pm%s & " % (floored_percentage(np.mean(elaterfdis),2), floored_percentage(np.std(elaterfdis),2)) + '\n')
